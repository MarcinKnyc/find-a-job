...
def insert_list_of_links(log_txt: str, error_txt: str, session: Session, links: List[str]):
    ...
    with open(log_txt, "a") as log_file:
        log_file.write(f"{datetime.now()} Successfully inserted {len(links)-failed_to_insert}/{len(links)} links to postgres. {failed_to_insert} failed.\n")
